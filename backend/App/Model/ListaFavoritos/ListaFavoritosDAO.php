<?php 
 ...
 class ListaFavoritosDao { 
     ...
     public function Create(ListaFavoritos $l) { 
  
        // podemos passar os valores no VALUES como (?,?) e depois colocar em sequência a numeração para inserir no banco de dados  
        $sql = 'INSERT INTO ListaFavoritos (id_usuario,mal_id,data_adicao,nota_pessoal,status) VALUES (?,?,?,?,?)'; 
        ...
        $stmt->bindValue(1, $l->getIdUsuario()); 
        $stmt->bindValue(2, $l->getMalId()); 
        $stmt->bindValue(3, $l->getDataAdicao()); 
        $stmt->bindValue(4, $l->getNotaPessoal()); 
        $stmt->bindValue(5, $l->getStatus()); 
     ...
     public function Read() { 
         ...
         $sql = 'SELECT * FROM ListaFavoritos'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->execute(); 
